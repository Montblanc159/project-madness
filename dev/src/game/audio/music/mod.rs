...
fn despawn_old_music(
    ...
    songs: Query<(Entity, &Song)>,
    mut events: MessageReader<PlaySong>,
) {
    for (entity, song) in songs {
        for event in events.read() {
            if song.title != event.song_title {
...
fn despawn_old_song_part(
    mut commands: Commands,
    song_parts: Query<(Entity, &SongPart)>,
    mut events: MessageReader<PlaySong>,
) {
    for (entity, song_part) in song_parts {
        for event in events.read() {
            if song_part.identifier != event.part {
                commands.entity(entity).despawn();
            }
        }
    }
    ...
    music_samples.par_iter().for_each(|music_sample| {
        match music_sample.audio_channel {
            AudioChannels::Rhythm => {
                rhythm_channel.stop();
                rhythm_channel.play(music_sample.file.to_owned()).looped();
            }
            AudioChannels::Bass => {
                bass_channel.stop();
                bass_channel.play(music_sample.file.to_owned()).looped();
            }
            AudioChannels::Melody => {
                melody_channel.stop();
                melody_channel.play(music_sample.file.to_owned()).looped();
            }
            AudioChannels::Extra => {
                extra_channel.stop();
